...
# -*- coding: utf-8 -*-
"""
copied from `Sharpe` @walkacross
https://github.com/StateOfTheArt-quant/sharpe/blob/main/sharpe/utils/risk.py
with avg_win_return, avg_loss_return, win_loss_ratio added
"""
...
class Risk(object):
    def __init__(self,
        ...
        self._win_rate = len(bar_returns[
            bar_returns > benchmark_bar_returns]) / self._period_count

        self._alpha = None
        self._beta = None
        self._avg_excess_return = np.mean(
            bar_returns) - self._daily_risk_free_rate
        self._sharpe = None
        self._return = np.expm1(np.log1p(bar_returns).sum())
        self._annual_return = (1 + self._return)**(self._annual_factor /
    ...
    @property
    def avg_win_return(self):
        return self._portfolio[self._portfolio > 0].mean()

    @property
    def avg_loss_return(self):
        return self._portfolio[self._portfolio < 0].mean()

    @property
    def win_loss_ratio(self):
        return abs(self.avg_win_return / self.avg_loss_return)
    ...
    def alpha(self):
        ...
        self._alpha = np.mean(self._portfolio - self._daily_risk_free_rate -
                              self.beta *
    ...
    def _calc_volatility(self):
        if len(self._portfolio) < 2:
            ...
            self._annual_volatility = self._volatility * (self._annual_factor**
                                                          0.5)
    ...
    def _calc_benchmark_volatility(self):
        if len(self._benchmark) < 2:
            ...
            self._benchmark_annual_volatility = self._benchmark_volatility * (
    ...
    def _calc_tracking_error(self):
        ...
        self._annual_tracking_error = self._tracking_error * (
            self._annual_factor**0.5)
    ...
    def information_ratio(self):
        ...
        self._information_ratio = np.sqrt(
            self._annual_factor
        ) * self._avg_tracking_return / self.tracking_error
        return self._information_ratio
    ...
    def sharpe(self):
        ...
        std_excess_return = np.sqrt((1 / (len(self._portfolio) - 1)) * np.sum(
            (self._portfolio - self._daily_risk_free_rate -
             self._avg_excess_return)**2))
        self._sharpe = np.sqrt(
            self._annual_factor) * self._avg_excess_return / std_excess_return
        return self._sharpe
    ...
    def excess_sharpe(self):
        ...
        std_excess_return = np.sqrt((1 / (len(self._portfolio) - 1)) * np.sum(
            (self._portfolio - self._daily_risk_free_rate -
             self.avg_excess_benchmark_return)**2))
        self._excess_sharpe = np.sqrt(
            self._annual_factor
        ) * self.avg_excess_benchmark_return / std_excess_return
        return self._excess_sharpe
    ...
    def _calc_downside_risk(self):
        ...
        self._annual_downside_risk = self._downside_risk * (self._annual_factor
                                                            **0.5)
    ...
    def excess_annual_return(self):
        if self._excess_annual_return is None:
            self._excess_annual_return = (1 + self._excess_return_rate)**(
                self._annual_factor / self._period_count) - 1
    ...
    def _calc_excess_volatility(self):
        if len(self._excess_portfolio) > 1:
            ...
            self._excess_annual_volatility = self._excess_volatility * (
                self._annual_factor**0.5)
        else:
    ...
    def performance(self):
        result = {
            ...
            'avg_win_ret': self.avg_win_return,
            'avg_loss_ret': self.avg_loss_return,
